...
public sealed class WpfCatalogGrpcService : CatalogGrpc.CatalogGrpcBase
{
    private readonly ICatalogService _catalogService;

    public WpfCatalogGrpcService(ICatalogService catalogService)
    {
    ...
    public override async Task<GetAlbumReply> GetAlbum(GetAlbumRequest request, ServerCallContext context)
    {
        ...
        return new GetAlbumReply { Album = Map(album) };
    }

    public override async Task<GetAlbumReply> CreateAlbum(CreateAlbumRequest request, ServerCallContext context)
    {
        var created = await _catalogService.CreateAlbumAsync(new AlbumUpsertDto(
            null,
    ...
    {
        var updated = await _catalogService.UpdateAlbumAsync(new AlbumUpsertDto(
            request.Id,
            request.VolumeNumber,
            request.Title,
            request.DargaudPublicationDate,
    ...
    private static AlbumItem Map(AlbumListItemDto album)
    {
    ...
    private static AlbumItem Map(AlbumDetailDto album)
    {
        var item = new AlbumItem
        {
            Id = album.Id,
